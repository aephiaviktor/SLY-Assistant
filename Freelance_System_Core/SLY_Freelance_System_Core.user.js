...
(async function() {
    ...
    async function addKeyToProfile(newKey) {
        ...
        // Requesting non-admin SAGE permissions for AEP automation, including addRemoveCargo for LM Market tests.
    ...
    function extractUserscriptVersion(scriptText) {
        let versionMatch = scriptText.match(/^\s*\/\/\s*@version\s+([^\s]+)\s*$/m);
        return versionMatch ? versionMatch[1] : null;
    }

    async function fetchUserscriptVersion(url, fallbackVersion) {
        let separator = url.includes('?') ? '&' : '?';
        let response = await fetch(url + separator + 'flyVersionCheck=' + Date.now(), { cache: 'no-store' });
        if (!response.ok) {
            throw new Error('HTTP ' + response.status);
        }
        let scriptText = await response.text();
        return extractUserscriptVersion(scriptText) || fallbackVersion;
    }

    async function loadFlyUpdateVersions() {
        let statusElem = document.querySelector('#updateVersionStatus');
        let slyaBtn = document.querySelector('#updateSlyaFlyBtn');
        let aepBtn = document.querySelector('#updateAepFlyBtn');
        if (!statusElem || !slyaBtn || !aepBtn) {
            return;
        }

        statusElem.textContent = 'Checking available versions...';
        slyaBtn.textContent = 'Update to SLYA FLY ...';
        aepBtn.textContent = 'Update to AEP FLY ...';

        let [upstreamResult, aepResult] = await Promise.allSettled([
            fetchUserscriptVersion(upstreamFlyUrl, upstreamFlyVersion),
            fetchUserscriptVersion(aepFlyUrl, flyVersion)
        ]);

        let availableUpstreamFlyVersion = upstreamResult.status === 'fulfilled' ? upstreamResult.value : upstreamFlyVersion;
        let availableAepFlyVersion = aepResult.status === 'fulfilled' ? aepResult.value : flyVersion;

        slyaBtn.textContent = 'Update to SLYA FLY ' + availableUpstreamFlyVersion;
        aepBtn.textContent = 'Update to AEP FLY ' + availableAepFlyVersion;

        if (upstreamResult.status === 'rejected' || aepResult.status === 'rejected') {
            statusElem.textContent = 'Could not refresh every version label. Buttons still open the latest raw scripts.';
        } else {
            statusElem.textContent = 'Available versions refreshed from GitHub.';
        }
    }

    async function updateToggle() {
        let targetElem = document.querySelector('#updateModal');
        let shouldOpen = targetElem.style.display === 'none';
        targetElem.style.display = shouldOpen ? 'block' : 'none';
        if (shouldOpen) {
            await loadFlyUpdateVersions();
        }
    }
    ...
    function waitForLabs(mutations, observer){
        ...
        if(document.querySelectorAll(elemTrigger).length > 0 && !document.getElementById("accountManagerContainer")) {
            ...
            updateModalContent.innerHTML = '<div class="assist-modal-header"><span>FLY Update</span><div class="assist-modal-header-right"><span class="assist-modal-close">x</span></div></div><div class="assist-modal-body" style="display: flex; flex-direction: column; gap: 10px; padding-top: 10px; padding-bottom: 12px;"><div>Current installed version: AEP FLY ' + flyVersion + '</div><div id="updateVersionStatus">Open this panel to check available versions.</div><button id="updateSlyaFlyBtn" class="assist-btn" style="width: 100%; padding: 8px;">Update to SLYA FLY ' + upstreamFlyVersion + '</button><button id="updateAepFlyBtn" class="assist-btn" style="width: 100%; padding: 8px;">Update to AEP FLY ' + flyVersion + '</button></div>';
            ...
            updateBtn.addEventListener('click', async function(e) {await updateToggle();});
            ...
            updateClose.addEventListener('click', async function(e) {await updateToggle();});
